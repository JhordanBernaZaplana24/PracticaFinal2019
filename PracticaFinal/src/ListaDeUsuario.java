...
public class ListaDeUsuario {
    ...
    public void escribir() {
        try (FileWriter fw = new FileWriter(ficheroDeGuardadoUsuarios)) {
            ...
            while (iterator.hasNext()) {
                fw.write(usuarios.get(iterator.next()).escribir());
            }
    ...
    public void leer() {
        try {
            ...
            String[] lineasDelUsuarioAcutual = new String[Usuario.LINEAS_PARA_LEER];
            while (iterator.hasNext()) {
                for (int i = 0; i < Usuario.LINEAS_PARA_LEER; i++) {
